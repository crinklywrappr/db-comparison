...
(def info
  {:db "Datalevin 1.0.0 (client/server)"
   :personality "networked · mutable · client/server · search built in"
   :caps {:valid-axis "derived from reified salary events"
          ...
          :search "native full-text index (server-side)"
          :migrations "syncopate (ragtime adaptor; applied-ids in a KV DBI on the same server env — never datoms); old attributes are truly REMOVED"}})
...
(defn- employee-tx [{:keys [name email hired dept role salary-history reviews]}]
  ;; Legacy (pre-split) shape — same as the embedded app. The seed runs between
  ;; migrations 001 and 002 (001 → [bounce] → seed → 002, see hr.main), so at
  ;; seed time :employee/name still exists; 002 (split-names) then rewrites these
  ;; rows into :employee/given-name + :employee/family-name.
